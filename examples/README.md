...
| 6 | [`manual_flow_control.py`](python/manual_flow_control.py) · [guide](../docs/site/guides/flow-control.md) | pause at the receive window and resume only after application consumption |
| 7 | [Event handling](../docs/site/guides/events.md) | what every event means and the action that normally follows it |
| 8 | [`graceful_shutdown.py`](python/graceful_shutdown.py) · [guide](../docs/site/guides/errors-and-shutdown.md) | two-stage GOAWAY while an accepted stream finishes |
...
| [`server_push.py`](python/server_push.py) | accepted push, promised stream mapping, disabled push, and delayed failure |
...
All examples except the live network client are deterministic and run in the
Python test matrix. Every example is formatted and linted, and the examples
directory is included in static type checking. The network client remains a
live integration check because remote availability, DNS, certificates, and
network policy do not belong to the package's deterministic test contract.
